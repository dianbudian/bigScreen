<template>
	...
	<div class="screen-page" :style="state.pageStyle">
		...
		<footer class="footer">
			...
			<div class="tec">
				<span style="margin-right: 10px;">数据来源：智慧蓉城“报表通”·新津区</span>
				<span style="margin-left: 10px;">技术支持：汇编科技</span>
			</div>
		</footer>
	</div>
</template>
...
state.amountFitClass = computed(() => {
	if (state.innerWidth <= 2022) {
		return "card-databar flex-databar";
	} else {
		return "card-databar";
	}
	return "card-databar";
...
// 大屏端
@media only screen and (min-width: 1920px) and (max-width: 2022px) {
    .card-databar {
        li {
            padding-top: 6px;
            padding-bottom: 6px;
...
@media only screen and (min-width: 1920px) and (max-width: 2259px) {
	.community-card {
		.village {
			li.item {
				margin-top: 10px;
